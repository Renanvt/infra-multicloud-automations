...
print_summary() {
    ...
    if [ "$ENABLE_HERMES" = true ]; then
        ...
        echo -e ""
        echo -e "  O Hermes Gateway foi instalado mas precisa ser configurado para subir."
        echo -e "  Um alias foi criado para facilitar — abra um novo terminal e use:"
        echo -e ""
        echo -e "  ${ARROW} ${BOLD}hermes setup${RESET}      — assistente interativo de configuração"
        echo -e "  ${ARROW} ${BOLD}hermes configure${RESET}  — configurar plataformas de mensagens"
        echo -e ""
        echo -e "  ${DIM}Ou manualmente: docker exec -it \$(docker ps -q -f name=hermes_hermes_gateway) hermes${RESET}"
        echo -e "  ${DIM}Dados em: /opt/infra/${BUSINESS_NAME}/hermes/${RESET}"
        echo -e ""
    fi
