...
setup_logging() {
    ...
    # Não redireciona stdout/stderr para evitar problemas com curl|bash
    # O log é feito via log_message() para o arquivo detailed.log
...
OPEN_DESIGN_OPENAI_KEY="${OPEN_DESIGN_OPENAI_KEY:-}"
...
print_banner() {
    ...
    echo -e "${CYAN}                                                                                                          ${RESET}"
    echo -e "${CYAN}                    CHATWOOT + EVOLUTION + GRAFANA + METABASE + DOCKER SWARM - v3.0                    ${RESET}"
    echo ""
}
